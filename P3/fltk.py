...
if TYPE_CHECKING:
    StatDictValue = TypedDict('StatDictValue', {"fichier_entree": str,
                                                "height": int,
                                                "width": int,
...
def image(
    ...
    __img_stats[img_object] = {"fichier_entree": fichier,
...
def capture_ecran(file: str) -> None:
    """
    Fait une capture d'écran sauvegardée dans ``fichier_entree.png``.
    """
    assert __canevas is not None
